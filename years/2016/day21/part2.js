const input = require('fs').readFileSync('./years/2016/day21/input.txt').toString().trim();
const common = require('../../../scripts/common');

module.exports = () => {
    let scrambleTheString = string => {
    ...
    let permutations = common.permutator('abcdefgh'.split('')).map(value => value.join(''));
    for (let i = 0; i < permutations.length; i++) {
        if (scrambleTheString(permutations[i]) == 'fbgdceah') return permutations[i];
    }
}